...
sudo sed -i "s:ReplaceSMTPServername:$email_smtp:g" $installdir/engine/confSettings.py
sudo sed -i "s:ReplaceSMTPServerPort:$email_smtp_port:g" $installdir/engine/confSettings.py

printf "\n"
printf "Please enter email address account (i.e. myemailaddress@gmail.com): \n"

read email_send_acct
sudo sed -i "s:ReplaceEmailSendAccount:$email_send_acct:g" $installdir/engine/confSettings.py

printf "\n"
printf "Please enter email address account password: \n"

read email_send_acct_pass
sudo sed -i "s:ReplaceEmailAccountPassword:$email_send_acct_pass:g" $installdir/engine/confSettings.py
...
sudo sed -i "s:ReplaceEmailFromAccount:$email_from_acct:g" $installdir/engine/confSettings.py
...
# Update settings file
sudo sed -i "s:ReplaceSMSAccountID:$sms_acct:g" $installdir/engine/confSettings.py

printf "\n"
printf "Please enter SMS Authentication Token (i.e. 82b546e6fdaab955b5bdaa53b37d84b9): \n"

read sms_auth_token
sudo sed -i "s:ReplaceSMSAuthToken:$sms_auth_token:g" $installdir/engine/confSettings.py
...
read sms_number
sudo sed -i "s:ReplaceSMSNumber:$sms_number:g" $installdir/engine/confSettings.py
